...
describe('MemoryStorage', () => {
  ...
  it('input body mutation after put does not affect stored data', async () => {
    const storage = new MemoryStorage();
    const originalBytes = new Uint8Array([1, 2, 3, 4, 5]);
    await storage.put({
      key: 'input-copy',
    ...
    const retrieved = await storage.get('input-copy');
    expect(retrieved!.body[0]).toBe(1);
  });
  ...
  it('input metadata mutation after put does not affect stored metadata', async () => {
    const storage = new MemoryStorage();
    const metadata: StorageMetadata = { version: '1.0' };
    await storage.put({
      key: 'meta-input-copy',
    ...
    const retrieved = await storage.get('meta-input-copy');
    expect(retrieved!.metadata['version']).toBe('1.0');
    expect(retrieved!.metadata['extra']).toBeUndefined();
  });
